...
#' @param weeks is the week you want to analyze
...
spde_inla <- function(path_lect, year = NULL, locality, path_coord,cve_edo,
    ...
    loc <- rgeomex::extract_locality(cve_edo = cve_edo,
                                     locality = locality)
    if(nrow(loc) > 1){
        loc <- loc  |>  sf::st_union() |> sf::st_as_sf()
    } else {
    ...
    mesh <- deneggs::fm_mesh(x = x, k = k, loc = loc, plot = plot)
    ...
    p <- as.data.frame(sf::st_coordinates(p)) |>
        dplyr::rename(x = X, y = Y) |>
        dplyr::mutate(mean = mod$summary.fitted.values[index, "mean"],
                      sd = mod$summary.fitted.values[index, "sd"],
                      lower = mod$summary.fitted.values[index, "0.025quant"],
                      upper = mod$summary.fitted.values[index, "0.095quant"],
                      week = weeks,
                      family = fam,
                      dic = mod$dic$dic)
